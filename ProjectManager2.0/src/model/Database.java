...
public class Database {
	...
	private void createTables() throws SQLException {	
		...
		String [] tableQueries = {
				...
				"CREATE TABLE IF NOT EXISTS Activities (ID INTEGER PRIMARY KEY AUTOINCREMENT,projectID INTEGER NOT NULL, Name TEXT UNIQUE,"
						+ "Description TEXT, Budget DOUBLE, StartDate TEXT, EndDate TEXT, Status TEXT);",
	...
	protected int validateProjectName(String name) {
		...
		try {
			...
			int count = 0;
			while (results.next()) {
				if(name.equals(results.getString(2))) //Email is NOT unique
					return 1;
			}
			System.out.println(count);
			return count;
...
*/
	...
	protected void addActivity(int projectID, String name, String description, double budget,String startDate, String endDate, String status) {

		try {
			String query = "INSERT INTO Activities (projectID, Name,Description,Budget,StartDate,EndDate,Status) VALUES (?,?,?,?,?,?,?);"; 
			prepStatement = connection.prepareStatement(query);
			prepStatement.setInt(1, projectID);
			prepStatement.setString(2, name);
			prepStatement.setString(3, description);
			prepStatement.setDouble(4,budget);
			prepStatement.setString(5,startDate);
			prepStatement.setString(6,endDate);
...
/*
	...
	protected ArrayList<Activity> getAllActivities(int projectID) throws Exception {
		...
		while(results.next()) {
			...
			Activity activity = new Activity(ID, pID, name, description, budget, startDate, endDate, status);
	...
	protected void updateActivity(int ID, String name, String description, double budget, String startDate, String endDate, String status) {
		try{
